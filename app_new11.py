...
if st.button("Clear Chat History", key="clear_all"):  # Sidebar or top
    st.session_state["chat"] = []
    st.session_state["next_entry_id"] = 0
    st.rerun()
...
def fetch_all_job_details(_client):
    ...
    for full in fetch_jobs_recursive(_client):
        try:
            ...
            # TEMP DEBUG: Print structure for first job (remove after testing)
            if not out:  # Only for first job to avoid spam
                print(f"DEBUG: Job '{full}' API structure keys: {list(info.keys())}")
                print(f"DEBUG: 'actions': {len(info.get('actions', [])) if info.get('actions') else 0} items")
                print(f"DEBUG: 'properties': {len(info.get('properties', [])) if info.get('properties') else 0} items")
                print(f"DEBUG: 'property': {len(info.get('property', [])) if info.get('property') else 0} items")
        except Exception as e:
            print(f"DEBUG: Failed to get info for {full}: {e}")
            continue
        desc = info.get("description", "") or "(no description)"
        params = []
        seen = set()
        # FIXED: Check ALL possible locations for parameterDefinitions (actions, properties, property)
        all_sections = (
            info.get("actions", []) +
            info.get("properties", []) +
            info.get("property", [])
...
def run_jenkins_job_and_get_output(job_name, params, client, poll_interval=2):
    try:
        for k, v in params.items():
            if isinstance(v, bool):  # From checkbox
                params[k] = str(v).lower()  # "true"/"false"
...
def llm_fast_match(query, client, top_n=10):
    ...
    valid_names_low = list(name_map.keys())

    # close = get_close_matches(query, valid_names, n=top_n, cutoff=0.3)
    matches_low = get_close_matches(query_low, valid_names_low, n=top_n, cutoff=0.3)
    return [name_map[m] for m in matches_low]
...
with chat_holder:
    ...
    for entry in st.session_state["chat"]:
        ...
        if entry.get("intent") == "jenkins":
            ...
            if matches:
                 # 1. Select job
                 sel = st.selectbox("Select Job", matches, key=f"jsel_{entry['unique_key']}")
                 ...
                 if job_details and job_details.get("parameters"):
                     ...
                     for i,param in enumerate(job_details["parameters"]):
                         p_name = param["name"]
                         p_type = param["type"].split('.')[-1]
                         p_default = param["default"]      
                         p_choices = param.get("choices", [])
                         widget_key = f"jenkins_param_{entry['unique_key']}_{sel}_{p_name}_{i}"       
                        
                         if "Boolean" in p_type:
                            default_bool = p_default.lower() == "true" if p_default else False
                            param_values[p_name] = st.checkbox(
                                label=f"**{p_name}** ({p_type})",
                                value=p_default == "true",  # Convert string default to bool
                                key=widget_key
                            )
                         elif ("Choice" in p_type or "Selection" in p_type) and isinstance(p_choices, list) and len(p_choices) > 0:
                        #  elif "Choice" in p_type:
                            # p_choices = param.get("choices", param.get("values", [])) or ["No choices defined"]
                            try:
                                idx = p_choices.index(p_default) if p_default in p_choices else 0
                            except:
                                idx = 0  # Default not in list? Use first choice
                            param_values[p_name] = st.selectbox(
                                label=f"**{p_name}** ({p_type})",
                                options=p_choices,
                                index=idx,
                                key=widget_key
                            ...
                            label_text = f"**{p_name}** ({p_type})"
                            if "Choice" in p_type:
                                label_text += " - [Dynamic/Manual Input]" 
                 ...
                 # 4. Run Button Logic
                 if st.session_state.get("last_jenkins_job") != sel:
                    # Clear all jenkins-related keys
                    for key in list(st.session_state.keys()):
                        if key.startswith("jenkins_param_") or key.startswith("param_"):
                            del st.session_state[key]
                    st.session_state.last_jenkins_job = sel
                 if st.button("Run Job", key=f"jrun_{entry['unique_key']}"):
