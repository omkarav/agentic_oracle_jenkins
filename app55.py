...
from oracle_runner import (
    ...
    get_snapshots_for_time, analyze_awr_report, compare_awr_reports ,run_full_health_check # ← ADD THESE
)
...
if "awr_history" not in st.session_state:
    st.session_state["awr_history"] = []  # list of dicts: {ts, label, report_html, snaps, period_str}
    ...
    for entry in st.session_state["chat"]:
        ...
        # NEW: AWR report flow
            ...
            if result.get("status") == "ok":
                ...
                # Save ONLY successful reports to history (no "Generating..." garbage)
                history_entry = {
                    "id": uid,
                    "ts": datetime.now(),
...
st.markdown("---")
st.subheader("Compare Any Two AWR Periods")

if len(st.session_state.awr_history) >= 2:
    # Clean list: only show successful reports, no duplicates
    ...
    if opt1["id"] != opt2["id"]:
        if st.button("Compare These Two Periods — Full Regression Analysis", type="primary", use_container_width=True):
            ...
            if comp["status"] == "ok":
                st.markdown("### Full AWR Comparison Report")
                st.markdown(comp["comparison"])
            else:
                st.error(comp.get("message", "Comparison failed"))
    else:
        st.info("Please select two **different** periods to compare.")
else:
    st.info("Generate at least 2 AWR reports to enable smart comparison.")
# ======================= FULL AI HEALTH CHECK BUTTON =======================
st.sidebar.markdown("---")
...
if st.sidebar.button("Health Check", type="primary", use_container_width=True):
    ...
    if health_data.get("status") == "ok":
        ...
        st.markdown(health_data["report"])
        
        # Optional: Download as PDF-like HTML
        st.download_button(
            "📄 Download Health Report (HTML)",
            data=health_data["report"],
            file_name=f"Oracle_Health_Check_{datetime.now().strftime('%Y%m%d_%H%M')}.html",
            mime="text/html"
        )
    else:
        st.error(health_data.get("message", "Health check failed"))
# -------------------------
