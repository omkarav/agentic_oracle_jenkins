...
.confidence-bar { background:#f0f0f0; height:4px; border-radius:2px; margin:4px 0; }
.confidence-fill { background:#4caf50; height:100%; border-radius:2px; }
...
# -------------------------
# Chat area: oldest -> newest (newest at bottom)
# -------------------------
...
# -------------------------
# Connected DB section moved to bottom
# -------------------------
st.markdown("---")
db_left_col, db_right_col = st.columns([0.7, 0.3])
with db_left_col:
    st.markdown("**Connected DB**")
    # allow selecting db (but connection only applies when user issues connect command or we allow connect button)
    db_select = st.selectbox("Select DB to connect (then click Connect):", options=st.session_state["dbs"], index=st.session_state["dbs"].index(st.session_state["current_db"]))
    if st.button("Connect", key="connect_db_bottom"):
        st.session_state["current_db"] = db_select
        # also add a system-like chat entry indicating the change
        append_chat_entry({
            "id": int(time.time() * 1000),
            "ts": time.strftime("%Y-%m-%d %H:%M:%S"),
            "request": f"Connect -> {db_select}",
            "intent": "connect_db",
            "oracle_sql": None,
            "oracle_result": {"status": "ok", "message": f"Connected to {db_select}"},
            "jenkins_matches": None,
            "jenkins_run": None,
        })
with db_right_col:
    st.markdown(f"<div class='db-badge'> {st.session_state['current_db']} </div>", unsafe_allow_html=True)
