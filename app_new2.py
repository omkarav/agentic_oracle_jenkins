...
def process_request_callback():
    ...
    if intent == "connect_db":
        ...
        patch_type = params.get("patch_type")
        patch_version = params.get("patch_version")
        
        if not patch_type:
            entry["oracle_result"] = {"status": "error", "message": "Could not parse patch type. Try 'download 19.28 DB RU'."}
        else:
            full_request = f"download {patch_type} {' '.join([patch_version] if patch_version else [])}"  # Reconstruct for parser
            with st.spinner("Updating config and starting download..."):
                res = download_patches(full_request, stream_output=True)
            
            if res["status"] == "running":
                entry["oracle_result"] = {"status": "ok", "message": res["message"]}
                st.session_state["patch_processes"][entry["id"]] = res["process"]  # Track for streaming
            else:
                entry["oracle_result"] = res
