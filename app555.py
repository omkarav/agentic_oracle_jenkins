...
def classify_intent(text):
    t = text.lower()
    if any(word in t for word in ["minute", "min", "now", "current", "right now", "recently", "slow", "hang", "issue", "problem", "performance"]):
        return "ash_report"
...
if len(st.session_state.awr_history) >= 2:
    ...
    opt1 = st.sidebar.selectbox(
        ...
        format_func=lambda x: x["short_label"],
        key="cmp_baseline"
    )
    ...
    if opt1["id"] != opt2["id"]:
        compare_clicked = st.sidebar.button("Compare These Two Periods — Full Regression Analysis", type="primary", use_container_width=True)
        if compare_clicked:
            with st.spinner("AI is comparing both AWR reports (SQLs, waits, load, regressions...)"):
            ...
            if comp["status"] == "ok":
                ...
                st.sidebar.success("Comparison complete! (View in main area)")
            else:
                st.session_state["awr_compare"] = None
                st.sidebar.error(comp.get("message", "Comparison failed"))
    else:
        st.sidebar.info("Please select two **different** periods to compare.")
else:
    st.sidebar.info("Generate at least 2 AWR reports to enable smart comparison.")
...
def process_request_callback():
    ...
    elif intent == "ash_report":
     minutes = parse_minutes(task)
     with st.spinner(f"Generating ASH Report - Last {minutes} minutes..."):
        result = generate_ash_report(minutes=minutes, db=st.session_state["current_db"])
    
     entry["oracle_result"] = result
     entry["oracle_sql"] = f"ASH Report - Last {minutes} minutes"
    
    # Save to history with clear label
     if result["status"] == "ok":
        history_entry = {
            "id": uid,
            "period_str": f"Last {minutes} mins",
            "label": f"{datetime.now().strftime('%H:%M')} - ASH Last {minutes}min",
